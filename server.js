...
const db = new Database(path.join(dataDirectory,"sandro-game.db"));
...
const dbPath = path.join(dataDirectory,"sandro-game.db");
...
const admin = findUser.get(ADMIN_USERNAME);
...
function getConfig(){
    return {
        runningText:getSetting.get("running_text")?.setting_value ||
            "SANDRO GAME V2",
        effect:getSetting.get("effect")?.setting_value || "rainbow"
    };
}
...
app.post(
    ...
    (req,res) => {
        ...
        const effect = allowedEffects.includes(req.body?.effect)
            ? req.body.effect
            : "rainbow";

        if(!runningText){
            return res.status(400).json({
                error:"Teks running tidak boleh kosong"
            });
        }
...
app.post(
    ...
    (req,res) => {
        ...
        if(game.score >= Math.floor(elapsedSeconds * 12)){
            return res.status(429).json({
                error:"Event terlalu cepat"
            });
        }

        updateScore.run(Date.now(),game.id);

        const updatedGame = getGame.get(game.id);

        res.json({
            score:updatedGame.score,
            level:updatedGame.level
        });
    }
);
